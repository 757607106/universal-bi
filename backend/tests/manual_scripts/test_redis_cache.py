...
def test_clear_cache():
    ...
    try:
        ...
        print(f"✓ 创建了 5 个测试缓存条目")
        
        # Check count
        pattern = f"bi:cache:{dataset_id}:*"
        keys_before = redis_client.keys(pattern)
        print(f"✓ 清除前缓存数量: {len(keys_before)}")
        ...
        # Verify
        keys_after = redis_client.keys(pattern)
        if len(keys_after) == 0:
            print("✓ 缓存清除验证通过")
        else:
            print(f"✗ 仍有 {len(keys_after)} 个缓存条目未清除")
...
def main():
    ...
    print(f"Cache TTL: {settings.REDIS_CACHE_TTL} 秒")
    print()
