...
## 更新摘要
**变更内容**   
- 新增业务术语管理功能，支持用户定义和训练业务术语、QA对和文档
- 增强训练流程进度跟踪，细化训练阶段和进度百分比
- 新增训练数据类型过滤功能，支持按类型查询训练数据
- 更新数据集模型，增加业务术语关系和训练日志
...
BTM["BusinessTermManager.vue<br/>业务术语管理"]
end
subgraph "后端"
...
- business_terms：业务术语关系，存储与数据集关联的业务术语。
...
Dataset "1" --> "many" BusinessTerm : "back_populates"
```
...
### 业务术语管理
- 前端通过 BusinessTermManager 组件管理业务术语，支持添加、删除和训练术语。
- 后端通过 add_business_term 接口处理术语添加，验证数据集权限并训练到 Vanna。
- 术语定义存储在 business_terms 表中，与数据集建立一对多关系。

```mermaid
sequenceDiagram
participant FE as "前端业务术语管理"
participant API as "API端点"
participant VM as "Vanna管理器"
FE->>API : 添加业务术语(addBusinessTerm)
API->>VM : train_term(dataset_id, term, definition)
VM-->>API : 训练结果
API-->>FE : 返回成功/失败
```
...
**图表来源**
- [BusinessTermManager.vue](file://frontend/src/views/Dataset/components/BusinessTermManager.vue#L1-L389)
- [dataset.py](file://backend/app/api/v1/endpoints/dataset.py#L178-L227)
